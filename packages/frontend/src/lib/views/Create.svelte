<script lang="ts">
	...
	import { Adapters, API, PayloadToLargeError, type FileDTO, type Note } from 'hashweb/shared'
	...
	async function submit(e: SubmitEvent) {
		...
		try {
			...
			try {
				const url = `${window.location.origin}/note/${response.id}`
				const fullUrl = result.password ? `${url}#${result.password}` : url

				const storedNote: StoredNote = {
					id: response.id,
					password: result.password,
					createdAt: Math.floor(Date.now() / 1000),
					expiresAt: timeExpiration
						? Math.floor(Date.now() / 1000) + (data.expiration! * 60)
						: undefined,
					viewsRemaining: !timeExpiration ? data.views : undefined,
					type: note.meta.type,
					url: fullUrl,
				}

				NoteStorageService.addNote(storedNote)
			} catch (e) {
