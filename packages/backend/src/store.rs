...
pub fn set_analytics(
    ...
    conn.set(key, serialized)
        .map_err(|_| "Unable to set analytics in redis")?;
    conn.expire(key, ttl_seconds)
        .map_err(|_| "Unable to set expiration on analytics")?;
    Ok(())
}

pub fn get_analytics(key: &String) -> Result<Option<Analytics>, &'static str> {
    let mut conn = get_connection()?;
    let value: Option<String> = conn
        .get(key)
        .map_err(|_| "Could not load analytics in redis")?;
    match value {
        None => return Ok(None),
        Some(s) => {
            let deserialize: Analytics = serde_json::from_str(&s).unwrap();
            return Ok(Some(deserialize));
        }
    }
}
