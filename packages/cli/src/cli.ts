...
const defaultServer = process.env['HASHWEB_SERVER'] || 'http://localhost:8000'
const server = new Option('-s --server <url>', 'the hashweb server to use').default(defaultServer)
...
program.name('hashweb').version(version).configureHelp({ showGlobalOptions: true })
